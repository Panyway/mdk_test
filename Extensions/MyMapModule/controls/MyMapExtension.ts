...
export class MyMapClass extends IControl {
	...
	public initialize(props: any): any {
		...
		if (this.definition().data.ExtensionProperties.Prop) {
			...
			this._storeInfo = this.definition().data.ExtensionProperties.Prop;
		}
		...
		if (app.android) {
			...
			var mapReadyCallBack = new com.google.android.gms.maps.OnMapReadyCallback({
				onMapReady: (gMap) => {
					...
					// that._gMap.addMarker(new com.google.android.gms.maps.model.MarkerOptions().position(latLng).title("location"));
					// that._gMap.moveCamera(new com.google.android.gms.maps.CameraUpdateFactory.newLatLng(latLng));
					for(var o of that._storeInfo){
						// var tmp = that._storeInfo[o];
						var address = o.HouseNumber + ' ' + o.Street + ' ' + o.City + ' ' + o.Country + ' ' + o.PostalCode;
						var data = that._geo.getFromLocationName(address, 1);
						var latLng = new com.google.android.gms.maps.model.LatLng(data.get(0).getLatitude(), data.get(0).getLongitude());
						that._gMap.addMarker(new com.google.android.gms.maps.model.MarkerOptions().position(latLng).title("location"));
						that._gMap.moveCamera(new com.google.android.gms.maps.CameraUpdateFactory.newLatLng(latLng));
					}
	...
	public view() {
		...
		this.valueResolver().resolveValue(this._storeInfo, this.context)
			.then((addresses) => {

				for(var o in addresses){
					var address = o.HouseNumber + ' ' + o.Street + ' ' + o.City + ' ' + o.Country + ' ' + o.PostalCode;

					if (app.ios) {
						return this.getlatlang(address)
							.then((cordinates) => {
								/* below code is for the apple maps */
								var latlong = CLLocationCoordinate2DMake(cordinates.latitiude, cordinates.longitude);
								var annotation = MKPointAnnotation.alloc().init();
	...
	public addMarker(customerAddress){
		var address = customerAddress.HouseNumber + ' ' + customerAddress.Street + ' ' + customerAddress.City + ' ' +
		customerAddress.Country + ' ' + customerAddress.PostalCode;
		console.log(address);
		if (app.android) {
			var data = this._geo.getFromLocationName(address, 1);
			var latLng = new com.google.android.gms.maps.model.LatLng(data.get(0).getLatitude(), data.get(0).getLongitude());
			this._gMap.addMarker(new com.google.android.gms.maps.model.MarkerOptions().position(latLng).title('Costco' +
								"'s " + "location"));			
		}
		if (app.ios) {
			return this.getlatlang(address)
				.then((cordinates) => {
					...
					var annotation = MKPointAnnotation.alloc().init();
					annotation.coordinate = latlong;
					annotation.title = "Costco" + "'s" + " location";
					this._mapView.centerCoordinate = latlong;
					this._mapView.addAnnotation(annotation);
				});
		}	
